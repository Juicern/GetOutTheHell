...
!function() {
    function e(t, e) {
        for (var o = 1, n = r(t), i = 0, a = c.length; i <= a; i++) {
            var s = 0;
            switch (e) {
              case u.L:
                s = c[i][0];
        ...
        if (o > c.length) throw new Error("Too long data");
        return o;
    }
    function r(t) {
    ...
    o.prototype = {
        ...
        for (var r = 0, o = 0, n = 0, a = new Array(e.length), s = new Array(e.length), h = 0; h < e.length; h++) {
            ...
            for (m = 0; m < a[h].length; m++) a[h][m] = 255 & t.buffer[m + r];
            r += u;
            var f = g.getErrorCorrectPolynomial(l), d = new i(a[h], f.getLength() - 1).mod(f);
        ...
        for (var v = new Array(p), T = 0, m = 0; m < o; m++) for (h = 0; h < e.length; h++) m < a[h].length && (v[T++] = a[h][m]);
        for (m = 0; m < n; m++) for (h = 0; h < e.length; h++) m < s[h].length && (v[T++] = s[h][m]);
        return v;
    };
    for (var h = {
        MODE_NUMBER: 1,
    ...
    (t = function(t, e) {
        ...
        for (var u = 0; u < n; u++) for (var l = 0; l < n; l++) {
            var g = o.isDark(u, l), f = r.padding ? l * i + r.padding : l * i, d = r.padding ? u * a + r.padding : u * a;
            t.setStrokeStyle(g ? r.colorDark : r.colorLight), t.setLineWidth(1), t.setFillStyle(g ? r.colorDark : r.colorLight), 
            t.fillRect(f, d, i, a), t.strokeRect(Math.floor(f) + .5, Math.floor(d) + .5, s), 
            t.strokeRect(Math.ceil(f) - .5, Math.ceil(d) - .5, h, s);
        }
        t.draw(!1, function() {
            setTimeout(function() {
                e.exportImage();
